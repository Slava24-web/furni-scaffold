...
export const CatalogProductSchema = z.object({
   ...
   */
  /**
   * Высота рабочей поверхности над низом модели.
   *
   * Обычно совпадает с высотой, но у столешницы в габарит входит
   * пристенный плинтус, а ставят на плиту: без отдельного числа мойка
   * повисает над столешницей.
   */
  surfaceHeightMm: z.number().int().positive().optional(),
  /**
   * Насколько изделие утоплено в опору, мм.
   *
   * Мойку не ставят НА столешницу — её врезают: снаружи остаётся бортик
   * заподлицо с камнем, а чаша уходит вниз, в тумбу. Без этого числа
   * мойка стоит ванночкой сверху и выдаёт себя с первого взгляда.
   *
   * В габарит коллизий утопленная часть не входит: она занимает объём
   * внутри модуля, который под неё и рассчитан.
   */
  ...
  /**
   * Окно, которое выпиливают в опоре под врезное изделие.
   *
   * Задаётся изделием, а не считается по габариту: у круглой мойки
   * бортик круглый, и прямоугольник «габарит минус припуск» вылез бы
   * из-под него углами — сквозь столешницу было бы видно тумбу.
   */
  cutout: z
    .object({
      widthMm: z.number().int().positive(),
      depthMm: z.number().int().positive(),
      /** Смещение окна от центра изделия: у мойки с крылом чаша сбоку */
      offsetXMm: z.number().int().default(0),
      offsetZMm: z.number().int().default(0),
    })
    .optional(),
  /**
