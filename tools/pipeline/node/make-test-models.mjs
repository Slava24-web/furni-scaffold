...
async function main() {
  ...
  for (const product of PRODUCTS) {
    ...
    manifest.products.push({
      ...
      /**
       * Роль в кухне: по ней считаются правила эргономики. Выводить её
       * из артикула нельзя — у реального тенанта артикулы свои
       */
      role: product.role,
      /** Пределы, в которых изделие тянется. Пусто — изделие не тянется */
      resize: product.resize ?? {},
      /**
       * Высота рабочей поверхности над низом модели. У столешницы это
       * плита без пристенного плинтуса: на плинтус ничего не ставят.
       */
      surfaceHeightMm: product.surfaceHeightMm ?? bounds.heightMm,
      /**
       * Насколько изделие утоплено в опору. Мойку врезают: её бортик
